...
Токены промпта и ответа не считаются: провайдер их возвращает, но
`llm.AssistantMessage` их не переносит. Считаются только токены эмбеддингов,
потому что `Embed` их уже отдаёт.

...
3. `Action` Determine what needs to be done
- Change
- Create
- Delete
- Update
...
4. `Location` Determine where the change needs to be made.
...
After determining which tool categories the task will need, call the `set_category` tool with those category names. Pick only from the following categories: {{ range $i, $c := .global.toolCategories }}{{ if $i }}, {{ end }}{{ $c }}{{ end }}. These categories determine which MCP tools are available to the planning stage when the user clicks Process Plan. Pick the smallest set that covers the whole task: every category you add puts all of its tools into the context of every stage planner, including the ones that will never call them. Leaving one out costs the planners a `tool_search` round, so err on the side of the surfaces the task plainly touches and no others. Each planner then chooses, per action it writes, which of them the agent that executes that action gets.
Draw DAG - `dag`.
...
Call these tools simultaneously: `create_summary`, `set_category`, `create_dag`, `create_plan_contract`
...
8. If you need more detailed information about the resource structure, you can use the tool `get_file_contents` to read the README.md in the root of the repository.

9. Return the response STRICTLY in Markdown format.
...
3. If stages, their order, or their dependencies changed, call `create_summary` and `create_plan_contract` in the same round with the refreshed summary and the whole rebuilt contract. Leave `set_category` alone unless the category set itself changed.
